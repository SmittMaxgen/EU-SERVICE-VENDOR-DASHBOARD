...
// ─── List item wrapper (keep original look) ───────────────────────────────────
    ...
    <List sx={{ width: '100%', maxWidth: { xs: 300, md: 330 }, py: 0 }}>
      {requests.map((request) => {
        ...
        return (
          <ListItemWrapper
            key={id}
            // ✅ Click on item → open detail on Notifications page
          ...
          >
            ...
            >
              ...
              <ListItemText primary={<Typography variant="subtitle1">{customer?.username || 'Unknown'}</Typography>} />
            </ListItem>
            ...
            <Stack sx={containerSX}>
              ...
              <Chip label={meta.label} color={meta.color} size="small" sx={{ width: 'min-content' }} />

              {/* ── Accept / Reject — stop propagation so click doesn't navigate ── */}
              {request_status === 'sent' && (
                <Stack direction="row" sx={{ gap: 1 }} onClick={(e) => e.stopPropagation()}>
                  <Button
                    ...
                    variant="contained"
                    color="success"
                    startIcon={<IconCheck size={13} stroke={2} />}
                    sx={{ fontSize: 11, py: 0.3, px: 1.5, minWidth: 0 }}
                    onClick={() => onUpdateStatus(id, 'accepted')}
                  >
                    Accept
                  </Button>
                  <Button
                    ...
                    variant="outlined"
                    color="error"
                    startIcon={<IconX size={13} stroke={2} />}
                    sx={{ fontSize: 11, py: 0.3, px: 1.5, minWidth: 0 }}
                    onClick={() => onUpdateStatus(id, 'rejected')}
                  >
                    Reject
